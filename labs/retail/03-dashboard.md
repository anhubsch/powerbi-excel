...
flowchart LR
    ...
    F --> G["Refresh: manual re-upload,<br/>not a live schedule"]
```
...
> BI Service re-pulling from a live database or API on a timer, so the
> report reflects today's data tomorrow. Here, "refresh" can only mean
> re-uploading the same fixed file, or manually re-running Power Query
> against a replacement export if one ever existed. Setting up a scheduled
> refresh against a `.xlsx` file that never changes would just be Power BI
> re-importing an identical file on a timer: technically functional,
> practically pointless. Worth saying directly rather than configuring
> a schedule that implies data is moving when it isn't.

### Step 3: What a real refresh setup would need instead

If this were a live retailer, the workflow would need: a gateway (On-
premises data gateway, if the source stays on a local file share) or a
cloud connector (if the source became a real database), a refresh schedule
matched to how often the source actually changes, and refresh failure
alerts. None of that applies to a fixed CSV export from 2011, so this lab
stops at publishing the static snapshot rather than building
...
2. What's the actual difference between "this report has scheduled
   refresh configured" and "this report's schedule fires against a file
...
- **Shipped the returns-rate table with conditional formatting before
  ...
  unreliable.
- **Set up a Power BI Service scheduled refresh against the static file**
  out of reflex, because that's the normal next step after publishing.
  Realized partway through configuring it that there was nothing for it to
  refresh against, the file doesn't change, and it would have just
  quietly re-imported the same 2011 data forever, implying freshness that
  didn't exist. Removed the schedule and documented the limitation instead.
