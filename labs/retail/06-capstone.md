...
- **Part 1:** Bring in a synthetic marketplace export in EUR and merge it
  against a currency conversion rate table in Power Query
...
live marketplace API, both sources are static files, and DirectQuery would
...
- `data/marketplace-orders-synthetic.csv`, which you generate yourself in
  Part 1 Step 1. There is no download for it, because it doesn't exist as
  real data anywhere.
- `data/fx-rates-synthetic.csv`, same, generated in Part 1 Step 2
...
### Step 1: Generate the synthetic marketplace data
...
Build this in Excel as a new synthetic dataset, not a download. There's no
real second-channel export for this retailer, because the scenario itself
is constructed for this lab. Base it on real products so it stays
plausible: pull 200-300 distinct `StockCode` values from Lab 02's
`dim_product`, generate 8,000-12,000 order rows across September-December
2011 (the marketplace channel launched partway through the year this
retailer's direct-site data covers), with `unit_price_eur` roughly the
GBP `UnitPrice` from `dim_product` multiplied by a plausible EUR/GBP rate
...
`RANDBETWEEN` for row-level randomness needs pasting over with values
immediately, exactly as Lab 02 Part 3 warned about for `dim_inventory`. A
moving synthetic dataset makes every downstream reconciliation in Part 5
impossible to trust. Generate once, paste values, and don't regenerate.

</details>

Save as `marketplace-orders-synthetic.csv`. Put "SYNTHETIC" in a comment
cell in row 1 of the source sheet before exporting, and again as a text
box on any report page that uses this data later.
...
`fx-rates-synthetic.csv`: `YearMonth`, `EUR_to_GBP_rate`, one row per month
from September through December 2011, rates plausible for that real period
...
**Get Data → Text/CSV** for both files. In Power Query, merge
`marketplace-orders-synthetic` against `fx-rates-synthetic` on a computed
`YearMonth` key (derived from `order_date`, same `YEAR()*12+MONTH()`
pattern as Lab 04's `YearMonthNumber`; reuse it rather than inventing a
second date-key convention). Add a computed column:
...
preview pane but never match in a merge.
...
`marketplace-orders-synthetic` loads with your generated row count
(8,000-12,000, per Step 1). Every row after the merge has a non-blank
...
channel's 3-4% cancellation rate, not dramatically higher or lower, per the
construction in Step 1.
...
**Close & Load**, `fact_orders_marketplace`.
...
- **Generated the marketplace `status` field with an even three-way
  ...
  actually just how I'd rolled the dice. Regenerated with "returned"
  weighted close to the direct channel's known 3-4%, and treated any
  remaining gap as the real signal worth writing up.
...
for. The DAX and the Power Query steps were just where that habit got the
most practice.
