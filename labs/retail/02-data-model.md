...
- **Part 1:** Import the cleaned workbook and shape a star schema
...
Lab 01 left off with a workbook that answers "which country, which
cancellation rate" through pivot tables, and a flag on `StockCode` still
to check: does the same product code ever carry more than one
`Description`? It does, and it's not a small problem: this retailer's
staff typed product descriptions by hand at the point of sale for over a
year, and the same SKU picked up spelling variants, case differences, and
outright different wording along the way.
...
- Power BI Desktop
- The `orders_with_flags` workbook from Lab 01
...
```mermaid
flowchart LR
    A[orders_with_flags.xlsx] --> B[Power Query in Power BI]
    B --> C[fact_orders]
    B --> D[dim_product<br/>StockCode drift resolved]
    B --> E[dim_customer<br/>incl. Guest row]
    B --> F[dim_country]
    B --> G[dim_date]
    H[Synthetic Excel formulas] --> I[dim_inventory]
    C --> J[Star schema]
    D --> J
    E --> J
    F --> J
...
**Home → Get Data → Excel** → `orders_with_flags.xlsx`, sheet
`orders_with_flags`. Load into Power Query, not directly onto the canvas:
the fact and dimension tables all come out of this one query.
...
The query editor shows the same row count as Lab 01's `orders_with_flags`
sheet, close to 540,000 rows, unchanged. This part only loads and plans;
...
> **Label synthetic data as synthetic, in the workbook, not just in this
...
### Step 2: Build it in Excel, one row per distinct StockCode

In a new sheet, pull the distinct `StockCode` list from `dim_product`
(Power Query → **Close & Load To** a connection-only table first, then
reference it here, or simply copy the distinct list once).

| Column | Formula approach |
|---|---|
| StockCode | From dim_product |
| warehouse_location | `=CHOOSE(RANDBETWEEN(1,3),"North","Midlands","South")` |
| stock_on_hand | `=RANDBETWEEN(0,500)` |
| reorder_point | `=RANDBETWEEN(20,100)` |

Paste values over the formulas once generated, so the numbers stop
changing every time the sheet recalculates: a what-if projection built on
a moving target in Lab 05 would be unusable.

### Step 3: Load it as its own table

**Close & Load To → Table**, sheet named `dim_inventory`. Import this
sheet into Power BI as a separate query, related to `dim_product` on
`StockCode`, one-to-one.

<details>
<summary>Hint</summary>

If Power BI flags the relationship as many-to-many instead of one-to-one,
`dim_inventory` still has duplicate `StockCode` rows. Go back to the Excel
sheet and confirm the distinct list in Step 2 was actually deduplicated
before the RANDBETWEEN columns were added, not after.
...
three values used in the CHOOSE formula. None of these numbers trace back
to the UCI dataset. Say so on the report page once this table reaches
...
Rename query `fact_orders`, **Close & Load**.
...
the UK total against the Excel pivot from Lab 01 Part 4. They should match
exactly, if they don't, the fact table grain or the cancellation flag
came through wrong somewhere in the import.

<details>
<summary>Hint</summary>

A mismatch usually traces to one of two places: `IsCancellation` imported
as text instead of a true boolean (check the column type in Power Query),
or `fact_orders` picked up extra rows from a bad merge somewhere in Part 1.
Compare row counts between the two tools before chasing the revenue figure
itself.
...
`Total Revenue` for the United Kingdom matches Lab 01's Excel figure to the
penny. `Cancellation Rate` overall lands in the same 3-4% range as the row
count identified in Lab 01 Part 1. `Net Revenue` is lower than `Total
Revenue` by roughly the value of cancelled orders. If `Net Revenue` comes
out higher, the cancellation quantities are being added instead of netted,
since they're stored as negative numbers by construction.
...
| Total Revenue in Power BI doesn't match Lab 01's pivot | IsCancellation didn't survive the Excel-to-Power-BI round trip as a proper boolean | Check the column type in Power Query; a text "TRUE"/"FALSE" won't filter the same way |
| dim_inventory relationship shows a many-to-many warning | Duplicate StockCode rows in dim_inventory | Confirm the distinct list step in Part 3 Step 2 actually deduplicated before pasting values |
...
- **Netted cancellations against original orders during the Power Query
  ...
  the Lab 01 workbook rather than un-net what I'd already collapsed.
- **Generated dim_inventory with live RANDBETWEEN formulas** and imported
  the sheet directly without pasting values first. Every time Power BI
  refreshed the query, the stock-on-hand numbers changed underneath the
  model, which made a what-if scenario in an early attempt at Lab 05
  impossible to reason about: the baseline itself was moving. Went back
  and pasted values.
