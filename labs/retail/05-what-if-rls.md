...
to low thousands of SKUs, depending on the random seed your `dim_inventory`
...
> generated with `RANDBETWEEN` in Lab 02 because the source dataset has no
> real stock table. A warehouse team reading this card without that
> context would reasonably assume it reflects an actual count. A text box
> stating the source plainly belongs next to this visual, not just in this
> lab's notes.
