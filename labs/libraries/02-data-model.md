...
- **Part 1:** Import the Lab 01 workbook into Power BI and split it into a
  star schema
...
- **Part 5:** Validate against the Lab 01 pivot
...
spelling differences in the `Title` field, and a plain pivot has no way to
tell those rows they're the same book.

Power BI fixes this differently than Excel would: a proper dimension table
built with a normalised join key groups those rows together once, and every
measure downstream inherits the fix automatically.

## Required Resources

- Power BI Desktop (free, Windows)
- `seattle-checkouts-2023-2025.xlsx` from Lab 01, containing
  `clean_checkouts` and `dim_material_type`
...
```mermaid
flowchart LR
    A[Excel workbook<br/>from Lab 01] --> B[Power Query in Power BI]
    B --> C[fact_checkouts]
    B --> D[dim_title]
...
**Get Data → Excel workbook** → tick `clean_checkouts` and
`dim_material_type` → **Transform Data**.

### Step 2: Build fact_checkouts

From `clean_checkouts`, keep: `Title`, `Creator`, `MaterialType`,
...
a subtitle. Lab 01 Part 4 Step 4 hit exactly this and set it aside; this is
where it gets fixed.
...
> **This is the exact trap Lab 01 flagged and did not fix.** A pivot table
> in Excel has no equivalent of a normalised join key. It can only group
> on the literal text value. Power BI's model doesn't fix this
> automatically either; the `TitleKey` transform is the fix, done once in
> Power Query, and every visual built against `dim_title` from here on
...
manually adding its fragments from the Lab 01 pivot.
...
> **This is the same gap Lab 01 hit with Title, generalised.** A pivot
> table's "% of subtotal" option only works within the pivot's own
> row/column structure. A measure using `ALL()` computes the same ratio
> but stays correct in any visual (a card, a matrix, a chart with a
> slicer applied) without redefining it per view.
...
of the sum you'd get by adding every MaterialType subtotal from the Lab 01
pivot (exact equality isn't guaranteed since Part 3's merge may have
grouped a couple of Lab 01's categories together). `Material Share` on an
unfiltered card reads 100%; filtered to one material type it reads well
under 100%. `Top Title Checkouts` should match whatever title led the Lab
01 Part 4 Step 4 pivot, now as one correct number instead of split
fragments.
...
the Lab 01 pivot subtotal exactly.
...
against the Lab 01 pivot's original category list.

### Step 2: Spot-check the title fix

Pick the title that was fragmented in Lab 01 Part 4 Step 4. Confirm its
Power BI total equals the sum of its fragments in the original Excel
pivot, not just one of the fragment rows.
...
| Total Checkouts doesn't match Lab 01 pivot | MaterialType merge in Part 3 changed grouping after the Lab 01 numbers were taken | Re-run the Lab 01 pivot against the Part 3 mapping, or treat Power BI's number as the corrected one and note why |
...
1. Why does grouping on a normalised `TitleKey` fix undercounting that
   Excel's pivot table couldn't, even though both tools are ultimately
   just grouping rows?
...
- **Merged two MaterialType values in Part 3 that were not actually the
  ...
  when the Material Share numbers didn't match Lab 01's original pivot
  and had to trace back which merge caused the drift.

---

## Where This Breaks

- The model works for one exported file, refreshed by hand
- No connection to the live open data portal. Someone has to re-filter,
  re-export, and re-point Power Query every time
