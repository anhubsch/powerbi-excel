...
Because the source is a local Excel file, this requires either the
**On-premises data gateway** (free download, installed on the same machine
as the file) or re-uploading the file manually.
...
> checkout data land there without anyone touching this workbook. A
> scheduled refresh only helps if something re-runs the Lab 01 filter and
> export against the live portal on a schedule, which a plain gateway
> refresh of a static local file does not do by itself. Publishing shares
> the report; it does not automatically go back and re-pull the source.
...
skipped. A more complete fix would call the portal's API directly from
Power Query instead of relying on a manually exported CSV at all; that's
worth naming as a known gap even if it's out of scope here.
...
is installed, a manual refresh from the dataset settings page should
complete without error and the "last refreshed" timestamp should update.
If no gateway is installed, the settings page should show refresh as
manual only, with no scheduled refresh option available, and that's the
correct state to document rather than a misconfiguration to chase.
...
| Scheduled refresh fails | No gateway, file source unreachable from the cloud | Install gateway, or accept manual refresh and document it |
...
3. Given that the source data genuinely updates monthly, is a manually
   re-exported CSV an acceptable long-term source for this dashboard?
...
- Refresh depends on a gateway running on one machine, or a human
  remembering to re-filter, re-export, and re-upload from the open data
  portal
- The model still has no way to compare this period to the same period
  last year, the reason Lab 04 exists
- Nothing prevents someone from editing the underlying Excel file in a way
  that silently breaks the title or material-type keys built in Lab 02
