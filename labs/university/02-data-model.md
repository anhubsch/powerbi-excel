...
- **Part 1:** Import the Lab 01 workbook into Power BI and build a star schema
...
pivot table and a manually-typed percentage column, and flagged, without
fixing, a real problem: some students appear more than once for the same
course in the same term. Left alone, that inflates whichever program those
duplicates happen to fall in.
...
- `university-data.xlsx` from Lab 01, containing `fact_enrollment`,
...
**Get Data → Excel workbook** → tick `fact_enrollment`,
`fact_library_visits`, `dim_student`, `dim_course`, `dim_term` →
**Transform Data**.

### Step 2: Rename and confirm column types

Power BI re-guesses types on import, independent of whatever was set in
Excel. Re-check `enrollment_date`, `checkout_date`, `return_date` are Date,
not Text. A Text-typed date column will still display fine in a table
visual and then fail silently the moment a relationship or a time
intelligence function needs it.
...
COUNTIFS helper column, but the order of magnitude should agree.
...
> **This is the same shape as Lab 01's gap.** A pivot table's manually
> typed percentage column has to be retyped for every new breakdown. This
...
all three terms, is close to the Lab 01 pivot's manually calculated number
for that program, not necessarily identical, since Lab 01's number still
included the duplicate rows.
...
The chosen program's Power BI completion rate and the Lab 01 pivot's
hand-calculated rate should sit within a percentage point or two of each
