...
source is a local Excel file, this needs either the **On-premises data
gateway** (free download, same machine as the file) or manual re-upload.

> **Publishing shares the report; it does not keep the data current.** A
> file-based source means either a gateway running permanently on one
> machine, or someone remembering to re-upload. For synthetic training
...
- Nothing prevents someone editing the underlying Excel file in a way that
  silently breaks the star schema
