...
`fact_enrollment` rows for Spring 2026, drawing `student_id` from the
existing 3,000 students and `course_id` from the existing 150 courses,
with `term_id = "T4"`. A realistic fraction of students don't re-enroll
every term: 5,000 rows across 3,000 students is a plausible continuation
rate, not full participation.
...
That's the actual argument for the star schema, the measure-not-pivot
