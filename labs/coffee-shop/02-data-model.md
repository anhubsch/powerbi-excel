...
bakery *share* is highest," a percentage-of-subtotal a plain pivot can't
give cleanly, and a manual recalculation for every new question.
...
- `coffee-shop-sales.xlsx` from Lab 01, containing `clean_sales` and
  `dim_product`
...
**Get Data → Excel workbook** → tick `clean_sales` and `dim_product` →
**Transform Data**.
...
> **This is the exact gap Lab 01 hit.** A pivot table's "% of subtotal"
> option only works within the pivot's own row/column structure. A measure
> using `ALL()` computes the same ratio but stays correct in any visual, a
> card, a matrix, a chart with a slicer applied, without redefining it.
...
you got from the Lab 01 pivot table for the same store/category filters
(small rounding differences are fine; a large gap means Part 2's key isn't
matching correctly). `Category Share` on a card with no filters applied
should read 100%; filtered to one category it should read something well
under 100%, never over.
...
manual percentage calculation, and it stays correct if you add a slicer,
...
01 pivot subtotal exactly.

**Troubleshooting:** a mismatch of a small, consistent amount usually means
Lab 01's duplicate transaction rows weren't fully removed before export.
Re-check the Excel workbook, not the Power BI model.
...
- The model works for one export file, refreshed by hand
- No connection to a real database, someone has to re-export and re-point
  Power Query every time
