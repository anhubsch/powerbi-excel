...
> it doesn't automatically keep the data current. A file-based source
> means either a gateway running permanently on your machine, or someone
> remembering to re-upload. That's the same "one person, one machine"
> fragility Lab 01 flagged, now visible one layer higher up.
...
3. Is a locally-gatewayed Excel file an acceptable long-term source for a
   dashboard a manager checks every morning? What would you tell them?
...
- Nothing prevents someone from editing the underlying Excel file in a way
  that silently breaks the star schema
