...
- **Part 1:** Import the Lab 01 workbook into Power BI and split it into a star schema
...
manual `GETPIVOTDATA` formula copied six times by hand: correct, but
fragile, and it told us nothing about wait times or how the synthetic
hospital compares to a real published benchmark.

It also left one thing unresolved: the department list was typed once, by
hand, into a lookup table. Real hospitals reorganize departments:
...
- `hospital-ops.xlsx` from Lab 01, containing `clean_appointments` and
  `clean_nhs_benchmark`
...
**Get Data → Excel workbook** → tick `clean_appointments` and
`clean_nhs_benchmark` → **Transform Data**.
...
Load `clean_nhs_benchmark` as-is, renamed `nhs_benchmark`. **Do not**
relate it to `fact_appointments` yet: it's monthly, trust-level, and
Lab 01 Part 2 Step 3 already flagged the grain mismatch. It stays an
unrelated table for now; Lab 04 handles the comparison properly, without a
relationship forcing a join that doesn't really exist.
...
Real trusts reorganize mid-year. Simulate it: in `raw_appointments` back in
Excel, for appointments scheduled after 1 July 2025 in what was
`Orthopaedics`, change the department label to `Trauma & Orthopaedics`:
same department, renamed partway through the year as part of a fictional
directorate merger. Re-save, re-import.
...
If `dim_department` still shows six rows after refreshing, the Excel
change didn't survive the round trip. Check you saved the workbook after
editing `raw_appointments`, and that `Home → Refresh` (not just reopening
the file) ran in Power BI.
...
you see blanks, a label in the raw data didn't match any `SourceLabel` in
the mapping table exactly (a stray space is the usual cause, same as Lab
01 Part 3 Step 3). Total row count in `fact_appointments` is unchanged
...
> **Why filter to `attended` explicitly.** `wait_minutes` is blank for
> no-shows and cancellations by construction (Lab 01 Part 1 Step 5).
...
Six No-Show Rate values clustered close to 12%, matching Part 4 of Lab 01
(they should agree closely, not exactly: Part 2 of this lab changed the
Orthopaedics label after Lab 01's manual figure was computed). Avg Wait
Minutes should land somewhere in the high teens to low twenties across
most departments, since Lab 01's `RANDBETWEEN(-5,45)` centres around 20.
...
here is close to the manual `GETPIVOTDATA` figure from Lab 01 Part 4.

**Troubleshooting:** if Orthopaedics/Trauma & Orthopaedics is the
department in question, the numbers won't match exactly. Lab 01's manual
figure was computed *before* the reorganization was introduced in Part 2
of this lab. That's expected, not a bug; note it rather than chasing a
...
- The model works for one exported workbook, refreshed by hand
