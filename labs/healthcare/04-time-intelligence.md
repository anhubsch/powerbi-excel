...
it never once looked at the real NHS benchmark file sitting unused in the
model since Lab 01.
...
- **Tried to relate `nhs_benchmark` directly to `dim_date` on exact date
  ...
  nothing, because the NHS file's period column parsed as the first of
...
- **Compared No-Show Rate directly against NHS 4-Hour Breach %** on the
  ...
  it once I reread Lab 01 Part 2 Step 3's own note about the grain and
  meaning mismatch.
